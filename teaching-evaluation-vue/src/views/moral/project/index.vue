      ...
      <div class="table-page">
        ...
        <el-table v-loading="loading" :data="pageResult.dataList" border style="width: 100%; margin-top: 10px;">
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="{row}">
              <el-button type="primary" v-if="!isStudent" size="mini" @click="handleUpdate(row)">
                编辑
              </el-button>
              <el-button v-if="isStudent" type="success" size="mini" @click="handleEvaluation(row)">
                评分
              </el-button>
              <el-button type="danger" v-if="!isStudent" size="mini" @click="handleDelete(row)">
                删除
              </el-button>
            </template>
          </el-table-column>
...
<script>
import { getProjectList, addProject, updateProject, deleteProject, submitEvaluation, getEvaluatedProjects, getUnevaluatedProjects } from '@/api/moral'
...
export default {
  ...
  methods: {
    ...
    async handleViewChange() {
      this.loading = true
      try {
        if (this.viewType === 'evaluated') {
          const res = await getEvaluatedProjects()
          if (res.code === 200 && res.data) {
            this.pageResult = {
              ...this.pageResult,
              dataList: res.data.evaluatedProjects || [],
              total: (res.data.evaluatedProjects || []).length
            }
          }
        } else if (this.viewType === 'unevaluated') {
          const res = await getUnevaluatedProjects()
          if (res.code === 200 && res.data) {
            this.pageResult = {
              ...this.pageResult,
              dataList: res.data.unevaluatedProjects || [],
              total: (res.data.unevaluatedProjects || []).length
            }
          }
        } else {
          await this.getList()
