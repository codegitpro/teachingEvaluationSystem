...
import com.geqian.evalution.teachingevalution.mapper.HealthEvaluationMapper;
...
@Service
public class HealthEvaluationServiceImpl extends ServiceImpl<HealthEvaluationMapper, HealthEvaluation>
    implements HealthEvaluationService {

  @Resource
  private UserService userService;

  @Resource
  private HealthProjectService projectService;
  ...
  @Override
  public ResponseResult<Object> submitEvaluation(HealthEvaluation evaluation) {
    // 获取当前用户信息
    ...
    // 检查是否已评分
    LambdaQueryWrapper<HealthEvaluation> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(HealthEvaluation::getProjectId, evaluation.getProjectId())
        .eq(HealthEvaluation::getStudentId, userId);
  ...
  public double getProjectAverageScore(Long projectId) {
    LambdaQueryWrapper<HealthEvaluation> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(HealthEvaluation::getProjectId, projectId);
    List<HealthEvaluation> evaluations = this.list(wrapper);
  ...
  public ResponseResult<Map<String, Object>> getEvaluatedProjects() {
    ...
    List<HealthProject> evaluatedProjects = this.baseMapper.selectEvaluatedProjects(userId);
    Map<String, Object> result = new HashMap<>();
  ...
  public ResponseResult<Map<String, Object>> getUnevaluatedProjects() {
    ...
    List<HealthProject> unevaluatedProjects = this.baseMapper.selectUnevaluatedProjects(userId);
    Map<String, Object> result = new HashMap<>();
