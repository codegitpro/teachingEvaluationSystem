...
import com.geqian.evalution.teachingevalution.mapper.MoralEvaluationMapper;
...
@Service
public class MoralEvaluationServiceImpl extends ServiceImpl<MoralEvaluationMapper, MoralEvaluation>
    implements MoralEvaluationService {

  @Resource
  private UserService userService;

  @Resource
  private MoralProjectService projectService;
  ...
  @Override
  public ResponseResult<Object> submitEvaluation(MoralEvaluation evaluation) {
    // 获取当前用户信息
    ...
    // 检查是否已评分
    LambdaQueryWrapper<MoralEvaluation> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(MoralEvaluation::getProjectId, evaluation.getProjectId())
        .eq(MoralEvaluation::getStudentId, userId);
  ...
  public double getProjectAverageScore(Long projectId) {
    LambdaQueryWrapper<MoralEvaluation> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(MoralEvaluation::getProjectId, projectId);
    List<MoralEvaluation> evaluations = this.list(wrapper);
  ...
  public ResponseResult<Map<String, Object>> getEvaluatedProjects() {
    ...
    List<MoralProject> evaluatedProjects = this.baseMapper.selectEvaluatedProjects(userId);
    Map<String, Object> result = new HashMap<>();
  ...
  public ResponseResult<Map<String, Object>> getUnevaluatedProjects() {
    ...
    List<MoralProject> unevaluatedProjects = this.baseMapper.selectUnevaluatedProjects(userId);
    Map<String, Object> result = new HashMap<>();
